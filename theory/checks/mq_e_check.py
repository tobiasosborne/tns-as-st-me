...
def require(cond, msg):
    """-O-safe gate (installed per blitz-mq-e-r1 minor 1)."""
    if not cond:
        print("FAIL:", msg)
        raise SystemExit(1)
    ...
    for lo in range(1, size - 1):
        for hi in range(lo, size - 1):
            ...
            for label in numeric_labels:
                ...
                require((all(state[x] == reference[x] for x in window)), 'invariant')
                for other_label in numeric_labels:
                    ...
                    if all(state[x] == other[x] for x in outside):
                        ...
                        require((other_moving in window), 'invariant')
                checks += 1
    return checks
...
def run(red: bool) -> tuple[int, int, int]:
    ...
    for size in range(7, 17):
        for mu in range(2, size - 3):
            ...
            require((set(expected.values()) == actual_component), 'invariant')
            require((len(expected) == size - 2), 'invariant')
            require((len(expected) == len(set(expected.values()))), 'invariant')
            ...
            require((actual_edges == claimed_edges), 'invariant')

            degree_histogram = Counter(
                sum(label in edge for edge in actual_edges) for label in expected
            )
            require((degree_histogram == Counter({2: size - 6, 1: 3, 3: 1})), 'invariant')
            require((len(walls(expected["d"])) == 1), 'invariant')
            require((all(
                len(walls(state)) == 3
            ...
            require(((mu + 1) - 1 == mu), 'incoming/reflected leg charge -1')
            require(((mu - 1) + 1 == mu), 'transmitted leg charge +1')
            require((mu == mu), 'side state is the pure kink K_mu')
