...
than renumbered so that the two live `AC-EX-2M-D29` pointers stay exact — its
where-proved cell excludes the vacated address `⟨1⟩9.⟨2⟩4`, whose proof
content is now at `⟨1⟩7′`, and its statement cell cites `⟨1⟩9.⟨2⟩5` for the
constructed-channel jet.  Renumbering would make the exclusion name that jet,
which does belong to `AC-EX-2M-D29`, and would strand the statement-cell
pointer.  The r6 convergence ruling therefore recommends retaining this
documented gap.
...
**Post-hoist locator update.**  The orchestrator has applied r5 §9 condition
2 and the subsequent pointer corrections: `AC-EX-2M` now cites `⟨1⟩7′` in
its statement, certifies D29-freeness through `⟨1⟩7′`, and has the simplified
where-proved cell `theory/ansatz-scattering-2m.md ⟨1⟩2--⟨1⟩7′`.  The mirror
above reproduces that live row.  The vacated `⟨1⟩9.⟨2⟩4` address remains only
because the live `AC-EX-2M-D29` row still excludes it and cites
`⟨1⟩9.⟨2⟩5` for the constructed-channel jet; retaining the marker preserves
those audited addresses without changing any proof content.
