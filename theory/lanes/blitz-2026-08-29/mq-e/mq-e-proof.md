...
**Status: PROVED-candidate.**  The theorem concerns the compressed operator
`H_3=P_3H_{m XXZ}P_3` of `memory-quantization.md` §3, not the unprojected
XXZ Hamiltonian.  In particular, it does not remove the separate
`P_3H_{m XXZ}(1-P_3)≠0` full-chain leakage problem.
