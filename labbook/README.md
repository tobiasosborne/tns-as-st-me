...
| `sections/18_continuum_reduction.tex` | `sec:continuum-reduction` | reduction to the accepted continuum statements (R1–R4), **and** the definitional audit against 3+1 compact lattice QED (Q1–Q4 lanes + adjudication): Gauss-law nucleus, memory verdict, soft-insertion verdict, ranked hostile findings, naming discipline. Physics-argument register throughout, not claims-DAG |
| `sections/19_operational_status.tex` | `sec:operational-status` | the operational wave (O1–O4 lanes + adjudication): the operational razor; why the injective-MPS boundary algebra passes; the operational registry (every D-number and load-bearing object as OBSERVABLE / IMPLEMENTABLE / GAUGE, with restate/flag actions); six explicit experiments; the exact non-identifiability of the ansatz gauge datum; the operator-algebraic boundary-observable literature, the Haag-duality import obligation and the citation debt; the one-species framing and the SYK/JT pure-gravity candidate with its decisive computation. Owns no statement; operational annotations + physics argument, not claims-DAG |
