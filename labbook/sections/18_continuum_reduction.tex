...
\subsection{One species, and the safe positioning}
\label{sec:red-one-species}

One framing point belongs here rather than in a later section, because it
governs how the reductions above should be read and how the work should be
positioned.

The continuum triangle is a \emph{two-species} statement. Matter radiates a
mediator; the mediator carries the soft leg; a matter detector records the
memory. Species separation is what makes the three corners look like three
different subjects. A one-dimensional spin chain has no second species, and the
campaign does not manufacture one. It replaces species separation by
\emph{superselection-sector separation}: kinks play the role of matter, magnons
the role of radiation, and the two are distinguished not by belonging to
different fields but by belonging to different sectors of the same field. Every
reduction above should be read with that substitution in mind --- in
particular, the observation that the ferromagnet computes what the kink model
cannot, and conversely, is a statement about two sectors of one theory, not
about two theories.

The substitution has an immediate corollary, and it is the discipline that
keeps the memory corner honest: \emph{memory needs a probe}. A permanent
coordinate change is not a memory effect; a permanent change in what a detector
records is. In a two-species world the probe is a piece of matter and the
carrier is the mediator. In a one-species world the probe must be a soliton of
the same field, which is exactly why the memory observable here is a domain
wall's position and not a test particle's displacement.

Hence the safe positioning of this work, stated once and plainly. What is on
offer is not a lattice copy of the gauge triangle, and it should never be
advertised as one. It is the \emph{assembly of the first one-species,
mediator-free triangle}, with sector separation doing the work that species
separation does in the continuum. Its nearest two-species antecedent is the
soft-pion triangle, which supplies the Adler-zero corner and no more: to the
best of the campaign's source-controlled searching no assembled one-species
triangle exists in the literature, and the assembly is the contribution. Its
pure-gravity sibling is the Schwarzian boundary mode of two-dimensional dilaton
gravity, which is a one-soft-species candidate in the same sense --- a single
boundary collective field carrying the infrared data --- and which comes with a
named decisive computation that has not been done. Pure gravity is therefore
\emph{inside} this map, not outside it. The corner-by-corner comparison, the
computation, and the honest list of what that setting still lacks are in
\cref{sec:operational-status}.

\provenance{---}{framing directive, owner 2026-08-30; corner map and
computation in \texttt{theory/lanes/reduction/o4-syk-jt-recon.md} \S4--\S5}{---}{\texttt{theory/verdicts/operational-wave-adjudication-r1.md} (A4)}
