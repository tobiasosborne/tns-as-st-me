...
across all two hundred sixteen measured rows.  The joint adversarial adjudication then did exactly what it exists to do:
it refused the strong conclusion. The referee found that the analytic
obstructions leave open a matter-dressed or explicitly relational
implementer on a separated, regulated domain; that the construction
lane's negative is a source-closure audit rather than a nonexistence
theorem; and that the microscopic measurement cannot isolate the failure
as specifically non-Möbius because its Möbius control mode is itself not
small in the accessible window. After one repair round applying the
referee's scope sentences verbatim, six exactly scoped statements entered
the claim ledger (identifiers \texttt{JT-A-tangent},
\texttt{JT-A-ward-grid}, \texttt{JT-A-uv-noexact}, \texttt{JT-A-purejt},
\texttt{JT-A-notconserved}, \texttt{JT-A-baredescent}): the mode-2 soft
tangent exists as an explicitly broken kinematic response and is not a
charge; the finite-grid measurement refutes the two implemented candidate
families only; no exact charge implements the literal primary rule on the
audited ultraviolet core; pure gravity without matter admits nothing
beyond functions of the Hamiltonian; any implementer of the mode-2
generator fails to be conserved; and the bare constraint expression does
not descend through the gauge constraint. The strong statement --- that
the symmetry corner fails at its first non-Möbius mode --- is explicitly
NOT claimed: the honest residual gap of the symmetry corner is the named
open route, a matter-dressed or relational implementer on a separated
regulated domain, which no lane constructed and no lane excluded. Before the lanes ran, the
